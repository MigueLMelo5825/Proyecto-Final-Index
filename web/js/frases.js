(function () {
  ...
  function init() {
    const mountEl = document.getElementById("quoteApp");
    if (!mountEl) return;

    new Vue({
      el: "#quoteApp",
      data: {
        literaryAndCinemaQuotes: quotes,
        currentQuote: null
      },
      methods: {
        displayRandomQuote() {
          const i = Math.floor(Math.random() * this.literaryAndCinemaQuotes.length);
          this.currentQuote = this.literaryAndCinemaQuotes[i];
        }
      },
      mounted() {
